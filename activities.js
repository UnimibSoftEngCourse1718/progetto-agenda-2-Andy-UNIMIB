...
{
  console.log ('scheduler reloadActivities for user: ['+ Agenda_OK.authuser +']');

  if (Agenda_OK.authuser)
  {
    var	trx = Agenda_OK.DBMS.DB.transaction ('activities', 'readonly').objectStore ('activities');
    ...
    trx.openCursor ().onsuccess = function (event)
    {
      ...
      if (cursor)
      {   
	if (cursor.value.id >= Agenda_OK.nextActivityID)
	  Agenda_OK.nextActivityID = cursor.value.id +1;
